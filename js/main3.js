...
const showProperty = () => {
  ...
  fetchElement('intro_title').innerHTML = `${selectedProperty}, ${state} ${postalcode} 
    <a style="color: #007bff;" href="https://www.google.com/maps/place/${selectedProperty}${city},${state} ${postalcode}"><i class="fa fa-map"></i></a>`;
  ...
  if (fetchElement('intro_tags').innerHTML === '') {
    amenities.map(({ Description }) => {
      fetchElement('intro_tags').innerHTML =
        fetchElement('intro_tags').innerHTML +
        `<li><a style="color: #838383;" href="#">${Description['#text']}</a></li>`
    });
  }
